// Card-image capture. The card's signature (devtier.com + @handle) is hidden on
...
// `.devtier-capturing`, which reveals the signature.
//
...
// Class added to the capture clone; `.devtier-signature` reveals under it.
export const SIGNATURE_CLASS = "devtier-capturing";
...
export async function renderCardImage<T>(
  ...
  if (opts.transparent) {
    ...
    clone.querySelectorAll<HTMLElement>(".devtier-card-frame").forEach((el) => {
      el.style.filter = "none";
    });
  }

  if (isWebKit()) {
    const avatar = clone.querySelector<HTMLElement>("[data-devtier-avatar]");
    // Measure off the LIVE node (laid out), not the clone (not yet in the DOM).
    const srcBox = node.querySelector<HTMLElement>("[data-devtier-avatar]");
