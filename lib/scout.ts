...
// Namespaced alongside devtier:scouts:total. The version segment lets a deploy
...
const keyFor = (login: string) => `devtier:card:${CACHE_VERSION}:${login}`;
...
export async function scoutCard(username: string): Promise<Card> {
  ...
  // Serve the in-memory sample cards by login so they resolve instantly and
  // work offline/without a token.
