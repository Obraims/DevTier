...
function rawStats(s: Signals): Stats {
  const o: Stats = {
    ...
    sho: 36 + 8 * Lg(s.total_stars_owned) + 3 * Lg(s.max_repo_stars) + 7 * Lg(s.recent_commits),
    pas: 40 + 12 * Lg(s.prs_to_others) + 6 * Lg(s.reviews) + 4 * Lg(s.followers),
...
function legacyScore(s: Signals): number {
  ...
  const z =
    ...
    g * Lg(s.total_contributions_lifetime) -
    f;
  return sigmoid(z);
}
...
export function buildCard(s: Signals): Card {
  ...
  const archetype = founder
    ? { name: "Founder", blurb: "co-founder of DevTier — they built the very scout reading this card" }
  ...
  return {
    ...
    account_age_years: s.account_age_years,
    total_stars_owned: s.total_stars_owned,
    recent_commits: s.recent_commits,
    prs_to_others: s.prs_to_others,
    reviews: s.reviews,
    followers: s.followers,
    languages: s.languages,
    issues_closed: s.issues_closed,
    total_contributions_lifetime: s.total_contributions_lifetime,
    recent_contributions: s.recent_contributions,
  };
}
