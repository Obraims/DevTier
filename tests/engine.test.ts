...
describe("buildCard — founder overrides", () => {
  it("forces obraims to a fixed overall above 89", () => {
    const card = buildCard(withLogin("obraims"));
  ...
  it("gives founders the dedicated 'founder' tier (not auto-promoted to icon)", () => {
    expect(buildCard(withLogin("obraims")).finish).toBe("founder");
    expect(buildCard(withLogin("mawsis")).finish).toBe("founder");
  });

  it("attaches per-founder art + accent to the card", () => {
    const founderCard = buildCard(withLogin("obraims"));
    expect(founderCard.founder).toBeDefined();
    expect(founderCard.founder?.art).toContain("founder-red");
    expect(founderCard.founder?.accent).toBe("#ff2f45");
