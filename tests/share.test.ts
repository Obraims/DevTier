...
describe("share service", () => {
  it("builds the canonical card URL from the login, encoding the displayed flag", () => {
    expect(cardUrl(card())).toBe("https://dev-tier.vercel.app/torvalds?country=us");
  });

  it("omits the country param when the card has no flag", () => {
    expect(cardUrl(card({ country: "" }))).toBe("https://dev-tier.vercel.app/torvalds");
  });
  ...
  it("X intent uses /intent/tweet (NOT /intent/post) and carries url + hashtag", () => {
    ...
    expect(u).toContain("hashtags=DevTier");
    expect(u).toContain(encodeURIComponent("https://dev-tier.vercel.app/torvalds?country=us"));
  });
  ...
  it("WhatsApp intent puts text + url in the message", () => {
    ...
    expect(decodeURIComponent(u)).toContain("dev-tier.vercel.app/torvalds?country=us");
  });
  ...
  it("native payload carries title, brag-led text, and url", () => {
    ...
    expect(p.title).toBe("DevTier");
    expect(p.url).toBe("https://dev-tier.vercel.app/torvalds?country=us");
...
describe("duel share service", () => {
  it("builds the canonical duel URL from both corners", () => {
    expect(duelUrl("obraims", "torvalds")).toBe("https://dev-tier.vercel.app/obraims/vs/torvalds");
  });

  it("message is deterministic per matchup, @-mentions the opponent, and never spoils the score", () => {
    const a = duelShareMessage("obraims", "torvalds");
    expect(a).toBe(duelShareMessage("obraims", "torvalds"));
  ...
  it("X intent uses /intent/tweet (NOT /intent/post) with the duel url + hashtag", () => {
    const u = duelIntentUrl("obraims", "torvalds");
    expect(u).toContain("https://twitter.com/intent/tweet?");
    expect(u).not.toContain("/intent/post");
    expect(u).toContain("hashtags=DevTier");
    expect(u).toContain(encodeURIComponent("https://dev-tier.vercel.app/obraims/vs/torvalds"));
  });

  it("native payload carries the title, the message, and the duel url", () => {
    const p = duelSharePayload("obraims", "torvalds");
    expect(p.title).toBe("DevTier Duel");
    expect(p.url).toBe("https://dev-tier.vercel.app/obraims/vs/torvalds");
    expect(p.text).toBe(duelShareMessage("obraims", "torvalds"));
  });
});
