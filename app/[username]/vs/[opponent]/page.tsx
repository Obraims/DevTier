...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if ("card" in a && "card" in b) {
    ...
    return {
      title: `${a.card.name} vs ${b.card.name} · DevTier Duel`,
  ...
  return {
    title: `@${username} vs @${opponent} · DevTier`,
    robots: { index: false },
  };
}
...
export default async function Page({ params }: Params) {
  ...
  after(() => Promise.all([recordScout(a.card.login), recordScout(b.card.login)])); // a duel is two scouts
