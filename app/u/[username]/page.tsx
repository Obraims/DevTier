...
export async function generateMetadata({ params }: { params: Promise<{ username: string }> }): Promise<Metadata> {
  ...
  if ("card" in res) {
    return {
      title: `${res.card.name} — ${res.card.overall} ${res.card.finishLabel} · DevTier`,
      description: `${res.card.name} scouted on DevTier: ${res.card.overall} OVR ${res.card.position}, ${res.card.archetype}.`,
  ...
  return { title: `@${username} · DevTier`, robots: { index: false } };
}
...
export default async function Page({
  ...
  if (card) {
    after(() => recordScout(username)); // analytics, flushed after the response (serverless-safe)
