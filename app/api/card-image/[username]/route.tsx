...
// Embeddable card image: devtier.com/<user>.png (via the next.config rewrite) -> here.
...
export async function GET(req: Request, { params }: { params: Promise<{ username: string }> }) {
  ...
  // Let embeds pin a flag: devtier.com/<user>.png?country=fr (the .png rewrite keeps
...
async function fallback(username: string) {
  ...
  return new ImageResponse(
    (
      ...
      >
        <div style={{ display: "flex", color: "#39d353", fontSize: 34, fontWeight: 700, letterSpacing: 6 }}>DEVTIER</div>
        ...
        <div style={{ display: "flex", marginTop: 10, fontSize: 32, color: "#39d353", fontWeight: 700 }}>dev-tier.vercel.app</div>
      </div>
    ),
