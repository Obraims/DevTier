...
const REPO_URL = "https://github.com/Obraims/DevTier";
...
export default function GithubStar({ stars }: { stars: number | null }) {
  ...
  const handleShare = async () => {
    const shareData = {
      title: "DevTier",
      text: "Rate any GitHub profile out of 99 as a FIFA-Ultimate-Team-style player card!",
  ...
  return (
    <div className="flex items-center gap-[8px]">
      <button
        type="button"
        onClick={handleShare}
        aria-label="Share DevTier website"
        className="group inline-flex items-center gap-[6px] rounded-full border border-line bg-bg-deep/55 px-[14px] py-[7px] text-[13px] font-semibold text-ink-soft backdrop-blur-md transition duration-200 hover:-translate-y-px hover:border-ink-mute hover:bg-bg-deep/80 hover:text-ink cursor-pointer outline-none"
      >
        {copied ? (
          <Check size={13} className="text-brand shrink-0" />
        ) : (
          <Share2 size={13} className="shrink-0 transition-transform duration-200 group-hover:scale-110" />
        )}
      ...
      <a
        ...
        target="_blank"
        rel="noopener"
        aria-label="Star DevTier on GitHub"
