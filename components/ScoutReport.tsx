...
function Section({
  ...
  return (
    <section className={`rounded-2xl border border-white/[0.06] bg-[#0b0930]/30 backdrop-blur-md p-[18px] shadow-[inset_0_1px_1px_rgba(255,255,255,0.03),0_8px_32px_0_rgba(0,0,0,0.2)] ${className ?? ""}`}>
      <div className="mb-[12px] flex items-center justify-between gap-[9px]">
        <div className="flex items-center gap-[9px]">
          <span className="h-[2px] w-[16px] rounded-full" style={{ background: accent, boxShadow: `0 0 8px ${accent}` }} />
          <h3 className="font-display text-[11.5px] font-bold tracking-[.22em] text-ink-faint">{title}</h3>
        </div>
        {headerRight}
      </div>
      {children}
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-3">
        <span className="text-[13px] font-medium text-ink-dim transition-colors group-hover/bar:text-white">{metric.label}</span>
        <span className="flex items-baseline gap-[6px]">
          <span className="text-[11px] tabular-nums text-ink-mute">
            {formatCount(metric.value)}
            {metric.unit ? ` ${metric.unit}` : ""}
          </span>
          <span className="font-display text-[16.5px] font-bold leading-none tabular-nums" style={{ color: accent, textShadow: `0 0 10px ${accent}44` }}>
            {metric.score}
          </span>
        </span>
      </div>
      <div className="mt-[7px] h-[4px] overflow-hidden rounded-full bg-white/[0.05] border border-white/[0.02]">
        <div
                ...
                <span className="inline-flex items-center gap-[6px] text-[clamp(11.5px,3vw,13px)] text-ink-dim">
                  ...
                  {card.languageLogo && (
                    <img
                      ...
                      alt=""
                      aria-hidden
                      className="h-[15px] w-[15px] object-contain pointer-events-none"
...
export function MetricsPanel({ card }: { card: Card }) {
  ...
  return (
    <Section
      title="SCOUTING METRICS"
      accent={accent}
      className="w-full"
      headerRight={
        <div className="flex items-center gap-1 rounded-lg bg-white/[0.02] p-0.5 border border-white/[0.05]">
          <button
            type="button"
            onClick={() => setView("bars")}
            className={`rounded px-2 py-0.5 text-[10px] font-bold tracking-wider transition active:scale-95 ${
              view === "bars" ? "bg-white/[0.08] text-white" : "text-ink-mute hover:text-ink-soft"
            }`}
          >
            BARS
          </button>
          <button
            ...
            className={`rounded px-2 py-0.5 text-[10px] font-bold tracking-wider transition active:scale-95 ${
              view === "radar" ? "bg-white/[0.08] text-white" : "text-ink-mute hover:text-ink-soft"
            }`}
          >
            RADAR
          </button>
    ...
    >
      {view === "bars" ? (
        ...
        <div className="mx-auto w-[82%] py-2">
          <StatRadar card={card} accent={accent} />
        </div>
      )}
    </Section>
  );
