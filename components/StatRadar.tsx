...
const getBreakdown = (card: Card, key: StatKey): string[] => {
  const activeYears = card.active_years ?? card.report.metrics.find((m) => m.label === "Active years")?.value ?? 0;
  const accountAge = card.account_age_years ?? card.report.metrics.find((m) => m.label === "Account age")?.value ?? 0;
  const totalStars = card.total_stars_owned ?? card.report.metrics.find((m) => m.label === "Stars earned")?.value ?? 0;
  const recentCommits = card.recent_commits ?? card.report.metrics.find((m) => m.label === "Commits")?.value ?? 0;
  const prsToOthers = card.prs_to_others ?? card.report.metrics.find((m) => m.label === "PRs submitted")?.value ?? 0;
  const reviews = card.reviews ?? card.report.metrics.find((m) => m.label === "Code reviews")?.value ?? 0;
  const followers = card.followers ?? card.report.metrics.find((m) => m.label === "Followers")?.value ?? 0;
  const languages = card.languages ?? card.report.metrics.find((m) => m.label === "Languages used")?.value ?? 0;
  const topLanguage = card.topLanguage ?? "N/A";
  const issuesClosed = card.issues_closed ?? card.report.metrics.find((m) => m.label === "Issues closed")?.value ?? 0;
  const totalContributions = card.total_contributions_lifetime ?? card.report.metrics.find((m) => m.label === "Contributions")?.value ?? 0;
  const recentContributions = card.recent_contributions ?? 0;

  switch (key) {
    case "pac":
      return [
        `${activeYears} active years`,
        `${Math.round(accountAge * 10) / 10} yrs account age`,
      ];
    case "sho":
      return [
        `${totalStars.toLocaleString()} total stars`,
        `${recentCommits.toLocaleString()} commits`,
      ];
    case "pas":
      return [
        `${prsToOthers} PRs submitted`,
    ...
    case "phy":
      return [
        `${totalContributions.toLocaleString()} lifetime`,
        `${recentContributions.toLocaleString()} recent contribs`,
      ];
    default:
      return [];
  }
...
export default function StatRadar({
  ...
  const rivalValue = activeKey && rival ? rival.card.stats[activeKey] : null;
  const read =
    rivalValue === null
      ? ""
      : value > rivalValue
    ...
    <div className="relative w-full">
      <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="w-full">
        ...
        {STATS.map((key, i) => (
          <polygon
            ...
            role="button"
            tabIndex={0}
            aria-label={`${STAT_LABELS[key]} ${stats[key]}${rival ? ` versus ${rival.card.stats[key]}` : ""}`}
      ...
      {activeLabel && activeKey && (
        ...
        >
          ...
          <div className="mt-1.5 flex flex-col gap-0.5 text-left border-t border-white/[0.08] pt-1">
            {getBreakdown(card, activeKey).map((item, idx) => (
              <span key={idx} className="text-[9.5px] text-ink-soft leading-normal font-medium whitespace-nowrap">
                • {item}
              </span>
            ))}
            {rival && rival.card && (
              <div className="mt-1 border-t border-white/[0.05] pt-1 flex flex-col gap-0.5 text-left">
                <div className="text-[8.5px] text-ink-mute font-bold tracking-wider uppercase mb-0.5">Rival:</div>
                {getBreakdown(rival.card, activeKey).map((item, idx) => (
                  <span key={idx} className="text-[9px] text-ink-mute leading-normal font-medium whitespace-nowrap">
                    • {item}
                  </span>
                ))}
              </div>
