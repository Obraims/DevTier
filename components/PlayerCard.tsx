...
import { resolveCardTheme, CARD_SKINS, type CardSkin } from "./finishTheme";
...
function PlayerCard({ card, skin = "classic" }: { card: Card; skin?: CardSkin }) {
  ...
  const wrap: CSSProperties = {
    ...
    width: "100%",
    aspectRatio: "540 / 820",
    filter: `drop-shadow(0 7cqw 10cqw rgba(0,0,0,.5)) drop-shadow(0 0 6cqw ${t.glow}) ${CARD_SKINS[skin]?.filter ?? ""}`.trim(),
  ...
  const skinStyle = CARD_SKINS[skin] ?? CARD_SKINS.classic;

  return (
    <div className="devtier-card-frame" style={wrap}>
      {skinStyle.overlayStyle && (
        <div
          style={{
            position: "absolute",
            inset: 0,
      ...
      {/* signature — maker's mark (bottom-left) + handle (bottom-right) on the
          ...
          clone (CardActions tags it `.devtier-capturing`), so it lands in every
          download / copy / share image but never on screen. See lib/capture.ts
          + the `.devtier-signature` rule in globals. */}
      <div className="devtier-signature">
        <div
          style={{
            ...at(8, 94.8),
            fontFamily: FONT_BOLD,
