...
function StatBar({
  ...
  const bar = (side: DuelSide, accent: string) => {
    ...
    return (
      <div
        className={`flex h-[6px] w-full overflow-hidden rounded-full bg-white/[0.04] ${
      ...
      >
        <div
          ...
          style={{
            ...
            background: lost 
              ? rgba(accent, 0.35) 
              : `linear-gradient(${side === "challenger" ? "270deg" : "90deg"}, ${accent}88, ${accent})`,
            boxShadow: won ? `0 0 12px ${accent}` : undefined,
            transition:
              "width .7s cubic-bezier(.16,1,.3,1), background .3s ease",
          }}
        />
      </div>
    );
  ...
  return (
    <div
      className="grid grid-cols-[minmax(30px,auto)_1fr_44px_1fr_minmax(30px,auto)] items-center gap-[9px] rounded-lg px-[8px] py-[8px] transition-all duration-200 hover:bg-white/[0.05] hover:shadow-[0_4px_12px_rgba(0,0,0,0.15)]"
      style={
        resolved
    ...
    >
      ...
      <span className="font-display text-center text-[10.5px] tracking-[.24em] text-ink-mute select-none">
        {row.label}
      </span>
      ...
      >
        ...
        >
          <div className="relative group">
            ...
            <div
              className="absolute -inset-1.5 rounded-[5.5cqw] opacity-0 blur-lg transition duration-700 group-hover:opacity-100"
              style={{
                background: `linear-gradient(135deg, ${theme.ink}, transparent 60%, ${theme.ink})`,
                opacity: won ? 0.8 : undefined,
                filter: "blur(16px)"
              }}
            />
        ...
        >
          <div className="w-[78%]">
            <StatRadar
              ...
              rival={
                side === "challenger"
                  ? { card: opponent, accent: bTheme.ink }
                  : { card: challenger, accent: aTheme.ink }
              }
            />
          </div>
      ...
      >
        ...
        <div className="mt-[clamp(14px,2.6vh,30px)] grid grid-cols-[1fr_minmax(270px,350px)_1fr] items-start gap-[clamp(14px,2.6vw,44px)] max-[900px]:flex max-[900px]:flex-wrap max-[900px]:items-start max-[900px]:justify-center">
          ...
          <div className="flex flex-col items-center max-[900px]:order-3 max-[900px]:w-full max-[900px]:max-w-[420px]">
            ...
            <div className="mt-4 flex items-center justify-center rounded-2xl border border-white/10 bg-[#0b0930]/40 backdrop-blur-md px-8 py-3 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]">
              <div className="font-display flex items-baseline gap-[clamp(12px,2vw,24px)] text-[clamp(64px,9.5vw,112px)] font-black leading-[.9]">
                <ScoreDigit value={scoreA} accent={aTheme.ink} />
                <span className="text-[0.45em] text-ink-mute select-none px-2">–</span>
            ...
            <div className="mt-[clamp(14px,2.2vh,20px)] w-full rounded-xl border border-white/[0.04] bg-white/[0.01] p-3 shadow-md">
              <div className="font-display flex items-center justify-between text-[11.5px] font-bold tracking-[.24em] text-ink-faint">
                <span className="tabular-nums" style={{ color: aTheme.ink, textShadow: `0 0 8px ${rgba(aTheme.ink, 0.3)}` }}>
                  {pctA}%
                </span>
                <span className="tracking-[.3em]">DOMINANCE</span>
                <span className="tabular-nums" style={{ color: bTheme.ink, textShadow: `0 0 8px ${rgba(bTheme.ink, 0.3)}` }}>
                  {100 - pctA}%
                </span>
              </div>
              <div className="mt-[9px] flex h-[8px] w-full overflow-hidden rounded-full bg-white/[0.05]">
                <div
                  className="h-full rounded-l-full"
                  style={{
                    width: `${pctA}%`,
                    background: `linear-gradient(90deg, ${aTheme.ink}88, ${aTheme.ink})`,
                    boxShadow: `0 0 10px ${rgba(aTheme.ink, 0.5)}`,
                ...
                <div
                  ...
                  style={{
                    background: `linear-gradient(270deg, ${bTheme.ink}88, ${bTheme.ink})`,
                    boxShadow: `0 0 10px ${rgba(bTheme.ink, 0.5)}`,
                  }}
                />
              </div>
            </div>
            ...
            >
              <section className="rounded-2xl border border-white/[0.06] bg-[#0b0930]/30 backdrop-blur-md p-[16px] shadow-[inset_0_1px_1px_rgba(255,255,255,0.03),0_8px_32px_0_rgba(0,0,0,0.2)]">
                <div className="mb-[12px] flex items-center justify-center gap-[9px]">
                  <span
                    className="h-[2px] w-[16px] rounded-full"
                    style={{ background: aTheme.ink, boxShadow: `0 0 6px ${aTheme.ink}` }}
                  />
                  <h3 className="font-display text-[11.5px] font-bold tracking-[.25em] text-ink-soft">
                    THE RECEIPTS
                  </h3>
                  <span
                    className="h-[2px] w-[16px] rounded-full"
                    style={{ background: bTheme.ink, boxShadow: `0 0 6px ${bTheme.ink}` }}
                  />
                </div>
                ...
                {duel.receipts.map((r) => (
                  <div
                    key={r.label}
                    className="grid grid-cols-[1fr_auto_1fr] items-baseline gap-[10px] border-b border-white/[0.06] py-[8px] last:border-0 hover:bg-white/[0.02] px-1.5 rounded-lg transition-colors"
                  >
