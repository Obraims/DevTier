...
import { confettiPalette, resolveResultTheme, CARD_SKINS, type CardSkin } from "./finishTheme";
...
export default function ResultView({
  ...
  useEffect(() => {
    let seen = false;
    try {
      seen = sessionStorage.getItem("devtier:seen-home") === "1";
    } catch {}
  ...
  return (
    ...
    <main className="relative z-[2] mx-auto flex min-h-[100dvh] w-full max-w-[1280px] flex-col px-[clamp(16px,4vw,22px)]">
      ...
      <div className="mb-[8px] mt-[clamp(8px,2vh,18px)] flex w-full shrink-0 items-center justify-between gap-[10px]">
        <div className="flex items-center gap-[10px]">
          <button
            ...
            className={
              seenHome
                ? "group inline-flex items-center gap-[6px] rounded-full border border-white/10 bg-white/[0.02] px-3.5 py-1.5 text-[12px] font-semibold tracking-wider text-ink-soft transition-all duration-200 hover:border-white/20 hover:bg-white/[0.06] hover:text-white"
                : "group inline-flex items-center gap-[6px] rounded-full border border-brand/20 bg-brand/[0.04] px-3.5 py-1.5 text-[12px] font-bold tracking-wider text-brand transition-all duration-200 hover:border-brand/40 hover:bg-brand/[0.08] hover:text-brand-hi"
            }
          >
            <ArrowLeft size={14} className="transition-transform group-hover:-translate-x-0.5" />
            <span>{seenHome ? "BACK" : "GET SCOUTED"}</span>
          </button>
          ...
          <div className="animate-walkout relative" style={{ width: CARD_WIDTH }}>
            <div ref={captureRef} className="relative">
              ...
              <div className="relative z-[1]">
                <PlayerCard card={card} skin={skin} />
              </div>
            </div>
            <FlagPicker value={card.country} onChange={onCountryChange} />
            {/* Skin Selector */}
            <div className="mt-4 flex flex-wrap items-center justify-center gap-1.5 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-2 backdrop-blur-md px-3 shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
              <span className="text-[9px] font-black tracking-[.2em] text-ink-faint uppercase mr-1.5">Skin</span>
              {(Object.keys(CARD_SKINS) as CardSkin[]).map((s) => {
                const active = skin === s;
                const skinTheme = CARD_SKINS[s];
                return (
                  <button
                    key={s}
                    type="button"
                    onClick={() => setSkin(s)}
                    className="relative flex h-[26px] items-center rounded-full border px-3 text-[10.5px] font-bold tracking-wider transition-all duration-250 active:scale-[.95]"
                    style={{
                      borderColor: active ? skinTheme.color : "transparent",
                      background: active ? `${skinTheme.color}26` : "rgba(255,255,255,0.02)",
                      color: active ? "#ffffff" : "var(--color-ink-soft)",
                      boxShadow: active ? `0 0 12px ${skinTheme.color}22` : undefined,
                    }}
                  >
                    {skinTheme.label}
                  </button>
      ...
      >
        <StoryFrame ref={storyRef} card={card} skin={skin} />
      </div>
    </main>
